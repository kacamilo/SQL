...
select
           to_char (max(hire_date), 'YYYY"년 "MM"월 "DD"일"')"마지막 입사일"
from employees;
...
예) 2001-01-13 토요일 */
select 
            to_char(min(hire_date), 'yyyy-mm-dd day' ) "가장 오래 근속한 직원"
from employees
group by hire_date
having min(hire_date) <= '01/01/13';
...
select hire_date,
        CASE 
                  WHEN hire_date<='02/12/31' and hire_date>='02/01/01' THEN '창립멤버'
                  WHEN hire_date<='03/12/31' and hire_date>='03/01/01' THEN '03년입사'
                  WHEN hire_date<='04/12/31' and hire_date>='04/01/01' THEN '04년입사'
                  ELSE '상장이후 입사'
        END optDate
from employees
